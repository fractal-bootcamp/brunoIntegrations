...
const CreateContactForm: React.FC = () => {
    ...
    const handleSubmitNewList = async (event: React.FormEvent) => {
        ...
        try {
            const token = await getToken();
            const newList = await createNewList(token, { name: listName, emails: emailsList, authorId: user.id });
            setNewList(newList)
                    ...
                    <div>
                        <ol>
                            {emailsList.map((item, index) => (
                                <div key={index}>
                                    <li >{item}</li>
                                    <button type="button" onClick={() => removeEmailFromList(item)}>x</button>
                                </div>
                            ))
                            }
                        </ol>
                        <label>
                            <button type="button" onClick={addEmailToList}>Add e-mail:</button> <input
                                type="text"
                                value={email}
